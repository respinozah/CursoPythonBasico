...
def main():
    ...
    if tienePermisoElUsuario(usuarioAuthenticado, "Consultar articulo"):
        print("2) Consultar detalles de un articulo")  
    ...
    if tienePermisoElUsuario(usuarioAuthenticado, "Imprimir inventario"):
        print("5) Imprimir el inventario completo")

    if tienePermisoElUsuario(usuarioAuthenticado, "Agregar articulo al carrito"):
        print("6) Agregar articulo al carrito")
...
def agregarAInventario():
    ...
    listaCategorias()
    categoria = str(input("Seleccione una categoria: "))

    listaSubCategorias(categoria)
    subCategoria = int(input("Seleccione una subcategoria:"))
    ...
    if existeArticulo(nombre):
        ...
        articulosSubcategoria.append(subCategoria)
        print(f"Se agregaron {cantidad} unidades de {nombre} al inventario con un valor de {precio} por unidad. Por favor usar la funcion de imprimir inventario para verificar.")
    input("\n\nPresione enter para continuar")
    main()

def consultarArticulo():
    os.system("cls")
    print("Modulo de inventarios\n----------------------------\nDetalles articulo\n")
    print("Articulos en inventario:")
    listarProductosDisponibles()
    detalleArticulo(int(input("\nIndique el articulo del que desea ver detalles: ")) - 1)
    input("\n\nPresione enter para continuar")
    main()
...
def detalleArticulo(indice):
    print(f"\nDetalle del articulo {articulosNombre[indice]}:")
    print(f"-Categoria: {categoriaArticulo(articulosNombre[indice])}")
    print(f"-Subcategoria: {subcategoriaArticulo(articulosNombre[indice])}")
    print(f"-Cantidad disponible: {articulosCantidad[indice]}")
    print(f"-Precio por unidad: {articulosPrecio[indice]}")
...
def subcategoriaArticulo(indiceArticulo):
    return subcategorias[articulosSubcategoria[articulosNombre.index(indiceArticulo)]]

def categoriaArticulo(indiceArticulo):
    for key, value in categorias.items():
        if value == subcategoriaArticulo(indiceArticulo):
            return key
            break
...
def listaSubCategorias(categoria):
    print(f"Subcategorias de {categoria}:")
    valoresCategoria = categorias[categoria]
    for key, valor in valoresCategoria.values():
        print(f"{key}) - {valor}")
...
def tienePermisoElUsuario(usuarioAuthenticado, permiso):
    ...
    if permiso == "Agregar articulo":
        if rolAsignado == 0 or rolAsignado == 1:
            tienePermiso = True
    elif permiso == "Consultar articulo":
...
def cargarTestData():
    ...
    #global categorias
    #categorias = {
    #    "Deportes": {0: 'Ciclismo', 1: 'Futbol', 2: 'Natacion'},
    ...
    subcategorias.append("Ciclismo")
    subcategorias.append("Futbol")
    subcategorias.append("Natacion")
    subcategorias.append("Entretenimiento")
    subcategorias.append("Educativo")
    subcategorias.append("Ciencia")
    subcategorias.append("Cocina")
    subcategorias.append("Dormitorio")
    subcategorias.append("Sala")
    categoriasSubCategorias.append(0)
    categoriasSubCategorias.append(0)
    categoriasSubCategorias.append(0)
    categoriasSubCategorias.append(1)
    categoriasSubCategorias.append(1)
    categoriasSubCategorias.append(1)
    categoriasSubCategorias.append(3)
    categoriasSubCategorias.append(3)
    categoriasSubCategorias.append(3)
    ...
    articulosSubcategoria.append(0)
    articulosSubcategoria.append(3)
    articulosSubcategoria.append(6)
    articulosSubcategoria.append(6)
